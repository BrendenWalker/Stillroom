...
This guide is to assist those installing Stillroom on Truenas Core using Docker and or Portainer

Docker install instructions adapted from [PhasedLogix IT Services's guide](https://getmethegeek.com/blog/2021-01-07-add-docker-capabilities-to-truenas-core/). Portainer install instructions adopted from the [Portainer Official Documentation](https://docs.portainer.io/start/install-ce/server/docker/linux). Stillroom installation on Portainer provided by users `Szeraax` and `TransatlanticFoe` on Discord (Thank you two!)
...
### 3. Install Stillroom VIA Portainer Web Editor
...
services:
  ...
  web_recipes:
    image: ghcr.io/brendenwalker/stillroom:latest
    env_file:
      - stack.env
    volumes:
...
-After those veriables are changed, you may press the `Deploy the Stack` button at the bottom of the page. This will create the needed containers to run Stillroom.

### 4. Login and Setup your new server!
- You need to access your Stillroom Server through its Webpage: `https://localhost:xxxx` replacing `localhost` with the IP of the VM running Docker and `xxxx` with the port you chose in the Web Editor for `nginx_recipes` above. In this case, `12008`. 
...
-You will now need to set up the Stillroom Server through the WebGUI.
