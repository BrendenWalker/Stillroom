...
This guide was contributed by [incaseoftrouble](https://github.com/incaseoftrouble).
...
The following could prove to be useful if you are not using Traefik, but instead run Apache as your reverse proxy to route all calls for a shared (sub)domain to a sub path, e.g. https://mydomain.tld/stillroom
...
```
    <Location /stillroom>
        ...
        # The following assumes that stillroom.service.consul.local resolves to the IP address of the Docker container.
        ProxyPass http://stillroom.service.consul.local:8080/stillroom
        ProxyPassReverse http://stillroom.service.consul.local:8080/stillroom
        RequestHeader add X-Script-Name /stillroom
        RequestHeader set X-Forwarded-Proto "https"
        ProxyPreserveHost On
    </Location>
    <Location /stillroom/static>
        Require local
        Require forward-dns [myhomedomain.useyourdomain.com]
        Require ip [anylocalorremoteipyouwanttowhitelist]

        ProxyPass http://stillroom.service.consul.local:8080/stillroom/static
        ProxyPassReverse http://stillroom.service.consul.local:8080/stillroom/static
        RequestHeader add X-Script-Name /stillroom
        RequestHeader set X-Forwarded-Proto "https"
        ProxyPreserveHost On
    </Location>
...
```
   stillroom:
     restart: always
     container_name: stillroom
     image: ghcr.io/brendenwalker/stillroom
     environment:
       - SCRIPT_NAME=/stillroom
       - STATIC_URL=/stillroom/static/
       - MEDIA_URL=/stillroom/media/
       - GUNICORN_MEDIA=0
       - SECRET_KEY=${YOUR_STILLROOM_SECRET_KEY}
       - POSTGRES_HOST=postgres.service.consul.local
       - POSTGRES_PORT=${POSTGRES_PORT}
       - POSTGRES_USER=${YOUR_STILLROOM_POSTGRES_USER}
       - POSTGRES_PASSWORD=${YOUR_STILLROOM_POSTGRES_PASSWORD}
       - POSTGRES_DB=${YOUR_STILLROOM_POSTGRES_DB}
     labels:
