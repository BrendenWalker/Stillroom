...
The docker image (`ghcr.io/brendenwalker/stillroom`) simply exposes the application on the container's port `80` through the integrated nginx webserver.
...
docker run -d \
    ...
    --name recipes_1 \
    ghcr.io/brendenwalker/stillroom
```
...
Follow the guide for installing DockSTARTer and then run `ds` then select 'Configuration' and 'Select Apps' to get Stillroom up and running quickly and easily.


!!!note
    DockSTARTer might not be updated for version 2 configurations
...
!!! danger
    Version 2 no longer build images for arm/v7 architectures. You can certainly get Stillroom working there but it has simply been to much effort to maintain these architectures over the past years
...
If you're having issues with installing Stillroom on your Raspberry Pi or similar device,
follow these instructions:

- Stop all Stillroom containers (`docker-compose down`)
- Delete local database folder (usually 'postgresql' in the same folder as your 'docker-compose.yml' file)
- Start Stillroom containers again (`docker-compose up -d`)
...
Version 1 includes gunicorn, a python WSGI server that handles python code well but is not meant to serve mediafiles. Thus, it has always been recommended to set up a nginx webserver 
(not just a reverse proxy) in front of Stillroom to handle mediafiles. The gunicorn server by default is exposed on port 8080.

Version 2 now bundles nginx inside the container and exposes port 80 where mediafiles are handled by nginx and all the other requests are (mostly) passed to gunicorn. 
