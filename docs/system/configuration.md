...
Port where Stillroom exposes its internal web server.
...
Stillroom supports HCAPTCHA which is supposed to be a privacy-friendly captcha provider.
...
Stillroom uses IP-based rate limiting on login, signup, and password reset to prevent brute-force attacks.
Because Stillroom's built-in nginx proxies to the application server via a unix socket, the application
cannot see client IP addresses directly and must read them from the `X-Forwarded-For` header.

This setting tells Stillroom how many reverse proxies sit between the client and the application server.
The default value of `1` accounts for Stillroom's built-in nginx and works for most deployments. If you
...
```
# prefix used for account related emails (default "[Stillroom] ")
...
S3_CUSTOM_DOMAIN= # when using a CDN/proxy to S3
```

#### AI Integration

Most AI features are configured through the AI Provider settings in the Stillroom web interface. Some defaults can be set for new spaces on your instance.
...
Stillroom's appearance can be changed on a user and space level but unauthenticated users always see the default style.
