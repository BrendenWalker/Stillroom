...
This means that you will be responsible for backing up your data.
...
The standard docker build of stillroom uses postgresql as the back-end database. This can be backed up using a function called "dumpall". This generates a .SQL file containing a list of commands for a postgresql server to use to rebuild your database. You will also need to back up the media files separately.
...
```
This connects to the postgres table instead of the actual djangodb table, as the import function needs to delete the table, which can't be dropped if you're connected to it.

## Backup using export and import
You can now export recipes from Stillroom using the export function. This method requires a working web interface.
1. Click on a recipe
2. Click on the three-dot menu then export
3. Select the all recipes toggle and then export. This should download a zip file.

Import:
Go to Import > from app > Stillroom and select the zip file you want to import from.
