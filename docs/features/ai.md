Stillroom has several AI based features. To allow maximum flexibility, you can configure different AI providers and select them based on the task you want to perform.
To prevent accidental cost escalation Stillroom has a robust system of tracking and limiting AI costs.
...
The models shown in the editor have been tested and work with Stillroom. Most other models that can parse images/files and return text should also work. 
...
Stillroom expects structured JSON responses from every AI call, so your model must support JSON mode or reliably return JSON.
...
- **JSON mode is required**: if the model ignores `response_format`, Stillroom will fail to parse the response and the request will error.
