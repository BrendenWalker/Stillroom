...
At Keycloak, create a new client and assign a `Client-ID`, this client comes with a `Secret-Key`. Both values are required later on. Make sure to define the correct Redirection-URL for the service, for example `https://stillroom.example.com/*`. Depending on your Keycloak setup, you need to assign roles and groups to grant access to the service.
...
By default, if a user signs in with a social provider whose email matches an existing Stillroom account, allauth will
...
To set up Stillroom with social login as the primary method and control space access via invite links:
...
!!! note
    ...
    (e.g., limit which users or groups can access the Stillroom application in Authentik/Keycloak).
...
**Login succeeds at provider but fails returning to Stillroom**
:   Check that the redirect URI configured at your provider matches your Stillroom URL exactly.
    For OpenID Connect providers, the callback URL is typically
    `https://your-stillroom-url/accounts/oidc/login/callback/` (or the provider-specific path).
