...
## Is there a Stillroom app?
Stillroom can be installed as a progressive web app (PWA) on mobile and desktop devices. The PWA stores recently accessed recipes locally for offline use.

### Mobile browsers

#### Safari (iPhone/iPad)
Open Stillroom, click Safari's share button, select `Add to Home Screen`

#### Chrome/Chromium
Open Stillroom, click the `add Stillroom to the home screen` message that pops up at the bottom of the screen

#### Firefox for Android
Open Stillroom, click on the `⋮` menu icon, then on `Install`

### Desktop browsers

#### Google Chrome
Open Stillroom, open the menu behind the three vertical dots at the top right, select `Install Stillroom...`

#### Microsoft Edge
Open Stillroom, open the menu behind the three horizontal dots at the top right, select `Apps > Install Stillroom`

## Why is Stillroom not working correctly?
If you just set up your Stillroom instance and you're having issues like;
...
| Header      | Requirement |
| :---        |    :----   |
| HTTP_HOST:mydomain.tld      | The host domain must match the url that you are using to open Stillroom.  |
| HTTP_X_FORWARDED_HOST:mydomain.tld      | The host domain must match the url that you are using to open Stillroom.  |
| HTTP_X_FORWARDED_PROTO:http(s)      | The protocol must match the url you are using to open Stillroom.  There must be exactly one protocol listed.  |
| HTTP_X_SCRIPT_NAME:/subfolder      | If you are hosting Stillroom at a subfolder instead of a subdomain this header must exist. |


## Why am I getting a 403 error when trying to log in?

Starting with version 2.6.1, Stillroom includes login rate limiting to protect against brute-force attacks.
This requires Stillroom to determine your client IP address from the `X-Forwarded-For` header set by
reverse proxies in front of the application.

Stillroom's built-in nginx is always counted as one trusted proxy, so the default setting of
...
to match your total number of proxies (including Stillroom's built-in nginx).

For example, if you run one external reverse proxy in front of Stillroom:
```
...
Stillroom has always rendered the recipe instructions markdown on the server. This also allows it to implement things like ingredient templating and scaling in text.
...
## Why is Stillroom not working on my Raspberry Pi?
...
## What are spaces?
Spaces are is a type of feature used to separate one installation of Stillroom into several parts.
...
For Stillroom that means all people that work together on one recipe collection can be in one space.
