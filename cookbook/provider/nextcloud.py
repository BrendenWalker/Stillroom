...
class Nextcloud(Provider):
    ...
    def import_all(monitor):
        ...
        if DEBUG:
            print(f'STILLROOM_PROVIDER_DEBUG checking path  {monitor.path} with client {client}')

        files = client.list(monitor.path)

        if DEBUG:
            print(f'STILLROOM_PROVIDER_DEBUG file list  {files}')

        import_count = 0
        for file in files:
            if DEBUG:
                print(f'STILLROOM_PROVIDER_DEBUG importing file {file}')
