...
def test_theming_function(space_1, u1_s1):
    ...
    assert get_theming_values(request)['theme'] == static('themes/stillroom.min.css')
    ...
    assert get_theming_values(request)['app_name'] == 'Stillroom'

    with scopes_disabled():
        up = UserPreference.objects.filter(user=request.user).first()
        up.theme = UserPreference.STILLROOM_DARK
    ...
    assert get_theming_values(request)['theme'] == static('themes/stillroom_dark.min.css')
    ...
    space_1.space_theme = Space.STILLROOM
